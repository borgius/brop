	...
	if (this._debug) {
		...
		this.log = function () {
			if (typeof console !== "undefined") {
				const args = Array.from(arguments, (arg) => {
				...
				const msg = Array.prototype.map
					.call(arguments, (x) => (x?.nodeName ? logNode(x) : x))
					.join(" ");
...
Readability.prototype = {
	...
	_getArticleTitle() {
		...
		if (/ [\|\-\\\/>»] /.test(curTitle)) {
			...
			const allSeparators = Array.from(origTitle.matchAll(/ [\|\-\\\/>»] /gi));
			...
			const headings = this._getAllNodesWithTag(doc, ["h1", "h2"]);
		...
		while (true) {
			...
			while (node) {
				...
				if (
					...
					const endOfSearchMarkerNode = this._getNextNode(node, true);
					let next = this._getNextNode(node);
					let itemPropNameNode = null;
					while (next && next !== endOfSearchMarkerNode) {
						const itemprop = next.getAttribute("itemprop");
						if (itemprop?.includes("name")) {
	...
	_unescapeHtmlEntities(str) {
		...
		return str
			...
			.replace(/&#(?:x([0-9a-f]+)|([0-9]+));/gi, (_, hex, numStr) => {
				let num = Number.parseInt(hex || numStr, hex ? 16 : 10);
		...
		this._forEachNode(scripts, function (jsonLdElement) {
			if (
				...
				try {
					...
					const content = jsonLdElement.textContent.replace(
						/^\s*<!\[CDATA\[|\]\]>\s*$/g,
						"",
					);
	...
	_getRowAndColumnCount(table) {
		...
		for (let i = 0; i < trs.length; i++) {
			...
			if (rowspan) {
				rowspan = Number.parseInt(rowspan, 10);
			}
			rows += rowspan || 1;
	...
	_cleanConditionally(e, tag) {
		...
		this._removeNodes(this._getAllNodesWithTag(e, [tag]), function (node) {
			...
			const isDataTable = (t) => t._readabilityDataTable;
			...
			if (this._getCharCount(node, ",") < 10) {
				...
				const headingDensity = this._getTextDensity(node, [
					"h1",
					"h2",
					"h3",
					"h4",
				...
				const textDensity = this._getTextDensity(node, textishTags);
