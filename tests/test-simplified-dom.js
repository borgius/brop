	...
	try {
		...
		for (const testPage of testPages) {
			...
			try {
				...
				console.log(`     Attribute selectors: ${selectorInfo.types.attribute.length}`);
				console.log(`     Contains selectors: ${selectorInfo.types.contains.length}`);
				console.log(`     Nth-child selectors: ${selectorInfo.types.nthChild.length}`);
			...
			try {
				...
				const selectorInfo = validateCSSSelectors(result.markdown || '');
				console.log(`   CSS selectors found: ${selectorInfo.total} (should be 0)`);
			...
			try {
				...
				console.log(`   Length: ${result.markdown?.length || 0} characters`);
				console.log(`   Source: ${result.stats?.source}`);
				console.log(`   Size increase from simple: ${
			...
			try {
				...
				if (result.stats?.byline) {
					...
					console.log(`     Excerpt: ${result.stats.excerpt?.substring(0, 100)}...`);
