...
async function runTests() {
	...
	try {
		...
		try {
			...
			console.log(`   Duration: ${duration}ms (~${Math.round(duration / result.typed.length)}ms per char)`);
		} catch (error) {
		...
		try {
			...
			console.log(`   Human-like mode: ${result.humanLike ? '✅ Enabled' : '❌ Disabled'}`);
		} catch (error) {
		...
		try {
			...
			console.log(`   Speed: ~${Math.round(longText.length / (duration / 1000))} chars/second`);
		} catch (error) {
		...
		try {
			console.log('   (This will take a moment to show realistic typing speed)');
			const result = await client.sendCommand('type', {
				tabId: tab1.tabId,
				selector: '[name="email"]',
