...
async function runDemo() {
	...
	try {
		...
		const demoPagePath = join(dirname(__dirname), 'tests/demo-typing-page.html');
		...
		const demoText = 'The quick brown fox jumps over the lazy dog. This demonstrates realistic typing!';
		...
		console.log(`   ✅ Completed in ${robotDuration}ms (${Math.round(robotDuration/demoText.length)}ms per char average)\n`);
		...
		console.log(`   ✅ Completed in ${humanDefaultDuration}ms (${Math.round(humanDefaultDuration/demoText.length)}ms per char average)`);
		...
		console.log(`   ✅ Completed in ${humanFastDuration}ms (${Math.round(humanFastDuration/demoText.length)}ms per char average)`);
		...
		console.log('   Watch the fourth input field for slow, deliberate typing...\n');
		...
		console.log(`   ✅ Completed in ${humanSlowDuration}ms (${Math.round(humanSlowDuration/demoText.length)}ms per char average)`);
		console.log(`   Corrections made: ${result4.corrections}\n`);

		// Summary
		console.log('📊 Summary of Typing Speeds:');
		console.log(`   Robot mode:      ${robotDuration}ms total (${Math.round(robotDuration/demoText.length)}ms/char)`);
		console.log(`   Human default:   ${humanDefaultDuration}ms total (${Math.round(humanDefaultDuration/demoText.length)}ms/char)`);
		console.log(`   Human fast:      ${humanFastDuration}ms total (${Math.round(humanFastDuration/demoText.length)}ms/char)`);
		console.log(`   Human careful:   ${humanSlowDuration}ms total (${Math.round(humanSlowDuration/demoText.length)}ms/char)`);

		// Keep tab open for observation
		console.log('\n📌 Demo complete! The tab will stay open for 30 seconds.');
		console.log('   Notice how the human modes have natural variations in speed.\n');
