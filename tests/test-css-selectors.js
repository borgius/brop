...
async function runTests() {
	...
	try {
		...
		try {
			...
			const hasDataTestIdSelectors =
				result.markdown?.includes("<!--[data-testid=");

			console.log(`   Has ID selectors: ${hasIdSelectors ? "✅" : "❌"}`);
			console.log(`   Has class selectors: ${hasClassSelectors ? "✅" : "❌"}`);
			console.log(
				`   Has aria-label selectors: ${hasAriaSelectors ? "✅" : "❌"}`,
			);
			console.log(
				`   Has data-testid selectors: ${hasDataTestIdSelectors ? "✅" : "❌"}`,
			);
			console.log("");
		} catch (error) {
		...
		try {
			...
			const selectorMatches = result.markdown?.match(/<!--([^>]+)-->/g) || [];
			console.log(
				`   Found ${selectorMatches.length} CSS selectors (should be 0)`,
			);
		...
		try {
			...
			const potentialFormLines = lines.filter(
				(line) =>
					line.includes("<!--") &&
			...
			if (inputMatches.length > 0) {
				console.log(
					`   Found ${inputMatches.length} inputs with expected format`,
				);
			}
			console.log("");
		} catch (error) {
