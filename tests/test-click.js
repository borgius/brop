...
async function runTests() {
	...
	try {
		...
		try {
			...
			console.log(`   Element: ${result.clicked.tagName} #${result.clicked.id}`);
			console.log(`   Text: "${result.clicked.text}"`);
			console.log(`   Position: x=${result.position.x}, y=${result.position.y}`);
			console.log(`   Size: ${result.position.width}x${result.position.height}`);
		} catch (error) {
		...
		console.log('🖱️ Clicking "More information" link with navigation detection...');
		try {
			const result = await client.sendCommand('click', {
				tabId: tab1.tabId,
			...
			console.log(`   Navigation occurred: ${result.navigation?.occurred || false}`);
		...
		try {
			...
			console.log(`   Checked state: ${element.element.checked ? '✅ Checked' : '⬜ Unchecked'}`);
		} catch (error) {
		...
		if (domContent.markdown?.includes('role="button"') || domContent.markdown?.includes('[role="button"]')) {
		...
		try {
			...
			console.log('✅ Correctly prevented click on disabled element:', error.message);
		}
		...
		console.log('   - Waits for elements to appear (with configurable timeout)');
		...
		console.log('   - Works with all clickable elements (links, buttons, checkboxes, etc.)');
