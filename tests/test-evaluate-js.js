...
async function runTests() {
	...
	try {
		...
		console.log('\n\n🧪 Test 3: Multiple statements with return');
		console.log('📝 Code: "const inputs = document.querySelectorAll(\'input\'); return inputs.length;"');
		...
		console.log('📝 Code: "async () => { await new Promise(r => setTimeout(r, 1000)); return \'waited 1 second\'; }"');
		...
		try {
			...
			if (result.result && typeof result.result === 'object') {
				...
				console.log(`   Properties: ${result.result.properties?.slice(0, 5).join(', ') || 'N/A'}...`);
			} else {
