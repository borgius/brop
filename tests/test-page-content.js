...
async function runTests() {
	...
	try {
		...
		try {
			...
			console.log("\n📊 Content validation:");
			console.log(
				`   Has DOCTYPE: ${content.html?.includes("<!DOCTYPE") ? "✅ Yes" : "❌ No"}`,
			);
			console.log(
				`   Has <html>: ${content.html?.includes("<html") ? "✅ Yes" : "❌ No"}`,
			);
			console.log(
				`   Has <head>: ${content.html?.includes("<head") ? "✅ Yes" : "❌ No"}`,
			);
			console.log(
				`   Has <body>: ${content.html?.includes("<body") ? "✅ Yes" : "❌ No"}`,
			);
		} catch (error) {
		...
		try {
			...
			if (content.html) {
				...
				console.log(
					`   Select field: ${hasSelectField ? "✅ Found" : "❌ Not found"}`,
				);
				console.log(
					`   Textarea: ${hasTextarea ? "✅ Found" : "❌ Not found"}`,
				);
			}
		} catch (error) {
		...
		try {
			...
			console.log(
				`   get_page_content HTML: ${pageContent.html?.length || 0} chars`,
			);
			console.log(
				`   get_simplified_dom HTML: ${simplifiedDom.html?.length || 0} chars`,
			);
			console.log(
				`   Reduction: ${Math.round((1 - (simplifiedDom.html?.length || 0) / (pageContent.html?.length || 1)) * 100)}%`,
			);
			console.log(
