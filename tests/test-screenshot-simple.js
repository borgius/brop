	...
	async connect() {
		return new Promise((resolve, reject) => {
			this.ws = new WebSocket(
				"ws://localhost:9225?name=screenshot_simple_test",
			);
...
async function runTest() {
	...
	try {
		...
		console.log(
			`Found ${tabList.totalTabs} tabs (${tabList.accessibleTabs} accessible)`,
		);
		...
		try {
			...
			console.log(
				`   Image data length: ${screenshot.image_data?.length || 0} characters`,
			);
			...
			if (screenshot.image_data) {
				...
				console.log(
					`   PNG signature: ${imageBuffer.slice(0, 8).toString("hex") === "89504e470d0a1a0a" ? "✅ Valid" : "❌ Invalid"}`,
				);
				console.log(
					`   Has content: ${imageBuffer.length > 1000 ? "✅ Yes" : "❌ Too small"}`,
				);
			}
		} catch (error) {
		...
		try {
			...
			if (screenshot.image_data) {
				...
				// JPEG validation
				console.log(
					`   JPEG signature: ${imageBuffer.slice(0, 3).toString("hex") === "ffd8ff" ? "✅ Valid" : "❌ Invalid"}`,
				);
			}
		} catch (error) {
