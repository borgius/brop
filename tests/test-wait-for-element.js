...
async function runTests() {
	...
	try {
		...
		try {
			...
			console.log(`   Element: ${result.element.tagName} #${result.element.id}`);
			console.log(`   Visible: ${result.element.isVisible ? '✅ Yes' : '❌ No'}`);
		} catch (error) {
		...
		const dynamicTestPath = join(dirname(__dirname), 'tests/test-wait-dynamic.html');
		...
		const visibilityTestPath = join(dirname(__dirname), 'tests/test-wait-visibility.html');
		...
		try {
			...
			console.log(`   Wait time: ${result.waitTime}ms`);
			console.log(`   Element is now visible: ${result.element.isVisible ? '✅ Yes' : '❌ No'}`);
		} catch (error) {
		...
		try {
			...
			console.log(`   Wait time: ${result.waitTime}ms`);
			console.log(`   Detection accuracy: ${Math.abs(result.waitTime - delay)}ms difference`);
		} catch (error) {
