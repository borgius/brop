...
Note: The extension will attempt to start `bridge/mcp-server.js` automatically on activation (uses the Node executable running VS Code). If you prefer to run the server manually, you can still run `node ./bridge/mcp-server.js` from the repository root; the extension will detect connection state from the server logs.
---
